  ...
  function renderRecommendations() {
      ...
      return `
        <div class="shopping-card mb-16" data-card-idx="${cardIdx}">
          ...
          <div class="shopping-card-actions mb-12">
            <button class="btn btn-primary btn-sm btn-full add-fridge-btn">
              🧊 냉장고에 추가 (구매 완료)
            </button>
          </div>
    ...
    resultEl.querySelectorAll('.shopping-card').forEach((cardEl) => {
      ...
      addBtn.addEventListener('click', async () => {
        // DOM 속성 왕복 대신 클로저의 rec 사용 — 특수문자로 속성이 깨져도 안전
        const name = rec.ingredient;
        const aiCategory = rec.category;
